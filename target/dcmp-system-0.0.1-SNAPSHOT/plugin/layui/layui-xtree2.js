...
layuiXtree.prototype.CreateCkAll = function () {
    ...
    if (_this._ckall) {
        ...
        _this._domStr += '<input  type="hidden" class="layui-xtree-checkbox layui-xtree-ckall" title="全选" lay-skin="primary" lay-filter="xtreeckall' + _this._containerid + '">';
        _this._domStr += '</div>';
    }
}
...
layuiXtree.prototype.dataBind = function (d) {
    ...
    if (d.length > 0) {
        for (var i = 0; i < d.length; i++) {
            var xtree_isend = 'disabled="disabled"';
            var xtree_ischecked = 'disabled="disabled"';
