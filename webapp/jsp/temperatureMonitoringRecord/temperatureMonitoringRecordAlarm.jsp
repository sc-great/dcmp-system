		...
		layui.use([ 'table', 'laydate', 'form', 'jquery' ], function() {
			...
			var startDate = laydate.render({
				...
				max : "2099-12-31",//设置一个默认最大值
					...
					var nowStrDate = date.year + "-" + date.month + "-"
					...
					var nowEndDate = endyear + "-" + endmonth + "-" + enddate
							+ " " + endhours + ":" + endminutes + ":"
							+ endseconds;
			...
			table.render({
				elem : '#user',
				height : 'full-120',
				url : '${path}/temperatureRecord/getRecordAlarm.do}',
				cellMinWidth : 80,
				cols : [ [ //标题栏
				{
					type : 'checkbox',
					...
					templet : function(d) {
						if (d.person.PBirth)
							return dateFormat(d.person.PBirth, 'yMd');
						else
							return '--';
					}
				}, {
				...
				add : function() {
					top.layer.open({
						...
						content : '${path}/jsp/user/user_add.jsp?areaId='
								+ '${param.areaId }' + '&areaName='
								+ decodeURI('${param.areaName }'),
						success : function(layero, index) {
							$(':focus').blur();
							layerIndex = index;
							layerInitWidth = $("#layui-layer" + layerIndex)
									.width();
							layerInitHeight = $("#layui-layer" + layerIndex)
									.height();
							resizeLayer($, layerIndex, layerInitWidth,
									layerInitHeight);
						},
						end : function() {
