...
<table id="sareaHost" lay-filter="sareaHostList"></table>
...
<script type="text/javascript">
    layui.use(['table', 'laydate', 'form', 'jquery'], function () {
        ...
        //监听工具条
        table.on('tool(sareaHostList)', function (obj) {
            var data = obj.data;
            if (obj.event === 'detail') {
                top.layer.open({
                    type: 2,
                    ...
                    isOutAnim: true,
                    resize: false,
                    content: '${path}/jsp/sareaHost/sareaHost_display.jsp?hostId=' + data.hostId,
                    end: function () {
// 							active.reload();
                ...
                top.layer.open({
                    ...
                    isOutAnim: true,
                    resize: false,
                    content: '${path}/jsp/sareaHost/sareaHost_update.jsp?hostId=' + data.hostId,
                    end: function () {
                        table.reload('sareaHost', {page: {curr: currpage}});
                    }
                });
            } else if (obj.event === "addMenu") {  //改为视频联动 ,暂未改
              
            } else if (obj.event === 'del') {
                let id = [data.areaHostId];
                //console.log(id);
                top.layer.confirm('确定要删除选中的数据吗？', {
                    btn: ['确定', '取消']
                }, function () {
                    var actionUrl = "${path}/sareaHost/del.do";
                    var params = {
                        ids: id
                    };
            ...
            active = {
                ...
                add: function () {
                    top.layer.open({
                        ...
                        isOutAnim: true,
                        resize: false,
                        content: '${path}/jsp/sareaHost/sareaHost_add.jsp',
                        success: function () {
                            $(':focus').blur();
                        },
                        end: function () {
                ...
                userNormal: function () {
                    var ids = active.getCheckData();
                    if (ids.length == 0) {
                        top.layer.alert('请选择需要启用的主机！！');
                    } else {
                        top.layer.confirm('确定要启用选中的主机吗？', {
                            btn: ['确定', '取消']
                        }, function () {
                            var actionUrl = "${path}/sareaHost/changeStatus.do";
                            var params = {
                                ids: ids,
                                status: 0
                            };
                ...
                userUnuser: function () {
                    var ids = active.getCheckData();
                    if (ids.length == 0) {
                        top.layer.alert('请选择需要停用的主机！');
                    } else {
                        top.layer.confirm('确定要停用选中的主机吗？', {
                            btn: ['确定', '取消']
                        }, function () {
                            var actionUrl = "${path}/sareaHost/changeStatus.do";
                            var params = {
                ...
                getCheckData: function () {
                    var data = table.checkStatus('sareaHost').data;
                    var ids = [];
                    for (var i = 0; i < data.length; i++) {
                        ids.push(data[i].hostId);
                    }
                    return ids;
                }
            };
